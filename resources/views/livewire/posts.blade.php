<div class="container mx-auto mt-2">
   <x-jet-banner />
    <div class="flex content-center m-2 p-2">
        <x-jet-button wire:click="showCreateModalPost" class="bg-green-500">Create Post</x-jet-button>
    </div>
      ...
      <div class="py-2 align-middle inline-block min-w-full sm:px-6 lg:px-8">
        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            @php
                $no = 1;
            @endphp
            @foreach ($posts as $tampilkan )

            <tr>
                <td class="px-6 py-4 whitespace-nowrap"> {{$no++}} </td>
                <td class="px-6 py-4 whitespace-nowrap">{{$tampilkan->title}}</td>
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                <img class="w-8 h-8 rounded-full" src=" {{asset('storage/photos/'. $tampilkan->image) }} " />
                <!-- <img class="w-8 h-8 rounded-full" src="https://picsum.photos/200" /> -->
                </td>
                <td class="px-6 py-4 text-center text-sm">
                    <x-jet-button wire:click="showEditPostModal ({{ $tampilkan->id }})" class="bg-blue-500">Edit</x-jet-button>
                    <x-jet-button wire:click="deletePost ({{ $tampilkan->id }})" class="bg-red-500">Delete</x-jet-button>
                </td>
            </tr>
            @endforeach
            <!-- More items... -->
    ...
    <x-jet-dialog-modal wire:model="showModalForm">
        ...
        <x-slot name="footer">
            @if($postId)
            <x-jet-button wire:click="updatePost"> Edit </x-jet-button>
            @else
            <x-jet-button wire:click="storePost"> Add </x-jet-button>
            @endif
